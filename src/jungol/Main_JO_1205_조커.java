...
public class Main_JO_1205_조커 {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			//if (ans == 540) break;
			//System.out.print("현재 인덱스 :"+now+" 남은 조커의 수 :"+j+" 현재  스트레이트 수 :"+temp +" 최대 스트레이트 수 :"+ans+"현재 카드 값 :" + seq.get(now)+"다음 카드 값 :"+ seq.get(next) +" 두 값이 같은가?");
			//System.out.println(seq.get(now).equals(seq.get(next)));
			...
			// 다음 카드 숫자와 현재 카드 숫자 사이에 들어갈 수 있는 숫자 개수 = (seq.get(next) - seq.get(now)) -1
				...
				now += 1; // 다음 카드로 이동
				next += 1; 
				if (preIdx == 0) { // 이전에 조커를 사용한 기록이 없는경우
